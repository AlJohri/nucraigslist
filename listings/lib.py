...
def save_obj(listing_obj):
    ...
    listing_obj['group_id'] = listing_obj['id'].split("_")[0]
    ...
    listing, listing_created = Listing.objects.update_or_create(id = listing_obj['id'], group_id = listing_obj['group_id'], message = listing_obj.get('message', ''), created_time = listing_obj['created_time'], updated_time = listing_obj['updated_time'], seller = user, picture = listing_obj.get('picture', ''), type = listing_obj['type'], object_id = listing_obj.get('object_id'))
    ...
    for comment_obj in listing_obj.get('comments', {}).get('data', []):
        ...
        commenter, commenter_created = User.objects.update_or_create(id=comment_obj['from']['id'], name = comment_obj['from']['name'])
        comment, comment_created = Comment.objects.update_or_create(id = comment_obj['id'], message = comment_obj.get('message'), created_time = comment_obj['created_time'], user = commenter, listing = listing)
    ...
    for liker_obj in listing_obj.get('likes', {}).get('data', []):
        liker, liker_created = User.objects.update_or_create(id=liker_obj['id'], name=liker_obj['name'])
